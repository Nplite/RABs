...
async def start_streaming(model_input: ModelInput,background_tasks: BackgroundTasks,current_user: User = Depends(get_current_active_user)):
    try:
        ...
        server_address = "http://192.168.3.5:8015"  # Update with your actual server address
...
async def start_single_camera_streaming(camera_id: str,model_input: ModelInput,background_tasks: BackgroundTasks, current_user: User = Depends(get_current_active_user)):
    try:
...
async def get_snapshots(year: int,  month: int, day: int, camera_id: int, current_user: User = Depends(get_current_active_user) ):
...
async def get_snapshots_by_month( year: int,month: int, camera_id: int, current_user: User = Depends(get_current_active_user) ):
...
async def get_snapshots_with_count(year: int,month: int,day: int,camera_id: int, current_user: User = Depends(get_current_active_user) ):
